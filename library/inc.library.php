...
# Fungsi untuk membuat kode Koleksi Inventaris buku (B0001.01, B0001.02, B0001.03, B0001.04)
function buatKodeInventaris($tabel, $inisial, $filter){
	$struktur	= mysql_query("SELECT * FROM $tabel");
	$field		= mysql_field_name($struktur,0);
	$panjang	= mysql_field_len($struktur,0);

 	$qry	= mysql_query("SELECT MAX(".$field.") FROM ".$tabel." WHERE kd_buku='$filter'");
 	$row	= mysql_fetch_array($qry); 
 	if ($row[0]=="") {
 		$angka=0;
	}
